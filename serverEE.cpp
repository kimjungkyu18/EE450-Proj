...
void parseCourses()
{
	...
	while (getline(ifs, line)) {
		...
		// cout << crs.code << "," << crs.credits << "," << crs.professor << "," << crs.days << "," << crs.name << endl;
	} 
}
...
string parseLine(string line)
{
	...
	if (tokens.size() == 1){  // This from a multiple course request, send ALL info
		cout << "MULTIPLE CASE REACHED!!" << endl;
...
int main()
{
	...
	while(1) {
		...
		result[res.length()] = '\0';
		cout << "Debug: Check if null terminated correctly: result=" << result << endl;
