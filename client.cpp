...
int main()
{
	...
	while(1) {
		...
		for (string course : courses){
			cout << "DEBUGGING, course vector: " << course << endl;
		}
		...
		if (courses.size() > 1){
			...
			cout << "DEBUGGING, multp: " << multp_course_line << endl;
			...
			cout << "Debugging recv: " << query_res << endl;
		}
			...
			printf("Debugging single course case. coursecode=%s, category=%s, queryline=%s\n", coursecode, category, query);
