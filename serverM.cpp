...
int main(void)
{
	...
	while(1) {
		...
		while (numTries < 3){
			...
			// Check if error or if client closed connection before authentication
			if ( (r = recv(clntsock, buf, sizeof(buf), 0)) == 0){
				// printf("Client has closed connection. Resume listening for connections.\n");
				goto accept;
			}
			if (r < 0) {
		...
		if (authenticated) {
			...
			while(1) {
				...
				char single_query[MAXLINE];
				strcpy(single_query, querystr.c_str());
				...
				if (courses.size() > 1){
					printf("The main server has received a multiple course request from %s.\n", user);
					...
					for (string course : courses){
						...
						// Non-EE/CS course
							...
							results.push_back("Didnt find course: " + course + "\n");
						}
					}
					// Aggregate query results to send to client
					string aggregated_res_str = "CourseCode: Credits, Professor, Days, Course Name\n";
					...
					size_t aggreg_len = strlen(aggregated_res_buf);
					aggregated_res_buf[aggreg_len] = '\0';
					if (send(clntsock, aggregated_res_buf, strlen(aggregated_res_buf)+1, 0) < 0) {
					...
					string query_str = "";
					query_str.append(tokens2.front()).append(",").append(tokens2.back());

					/*
					// Parse query
					char code[tokens2.front().length() + 1];
					char category[tokens2.back().length() + 1];
					char query_buf[MAXLINE];
					memset(query_buf, 0, sizeof(query_buf));

					strcpy(code, tokens2.front().c_str());
					code[tokens2.front().length()] = '\0';
					strcpy(category, tokens2.back().c_str());
					category[tokens2.back().length()] = '\0';
					strcpy(query_buf, query_str.c_str());
					query_buf[query_str.length()] = '\0';
					*/
					...
					strcpy(code, tokens2.front().c_str());
					strcpy(category, tokens2.front().c_str());
					...
					if (strncmp(code, "EE", 2) == 0) {
						if ( sendto(udpsock, single_query, sizeof(single_query), 0, (struct sockaddr *) &servEE_addr, sizeof(servEE_addr)) < 0 ){
							fprintf(stderr, "ERR: sendto query failed\n");
						}
						printf("The main server sent a request to serverEE.\n");
					...
					// Send to CS server
						...
						if ( sendto(udpsock, single_query, sizeof(single_query), 0, (struct sockaddr *) &servCS_addr, sizeof(servCS_addr)) < 0 ){
							fprintf(stderr, "ERR: sendto query failed\n");
						}
						printf("The main server sent a request to serverCS.");
